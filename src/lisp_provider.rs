...
"#;
    ...
    ///
    /// Only [`Self::to_actual_str`] is defined here: figment's [`Value`]
    /// exposes inherent `to_i128` (`figment/src/value/value.rs:261`) and
    /// `to_bool` (generated by the `conversion_fn!` macro at
    /// `figment/src/value/value.rs:222`), so a trait method with either
    /// of those names would be shadowed by inherent-method resolution and
    /// stay unreachable — the `dead_code` warning at this file's own
    /// `pending-shikumi-clippy` note (`CLAUDE.md`) that carrying the two
    /// dead trait methods produced was the sole surviving warning on
    /// `cargo build --all-features`, blocking a future measured
    /// `-D warnings` gate in CI. There is no inherent `to_actual_str`
    /// on figment's [`Value`] (`as_str` returns a `&str` but only for
    /// the [`Value::String`] variant, and the test call sites already
    /// name `to_actual_str`), so keeping that one method leaves the
    ...
    /// Pin the load-bearing invariant that keeps the two dead trait
    /// methods deleted: `Value::to_i128` and `Value::to_bool` MUST
    /// resolve to figment's inherent methods (not through any
    /// `ValueHelpers`-like trait in this module's scope), so the
    /// call sites at lines 321 / 322 / 355 / 356 / 380 / 381 stay
    /// intact after the removal. If a future figment upgrade removes
    /// either inherent method, this test starts failing with a
    /// resolution error — the signal to either re-add the trait
    /// method or migrate the call sites to the surviving inherent
    /// (`to_bool_lossy`, `to_num_lossy().and_then(Num::to_i128)`, …).
    #[test]
    fn figment_value_carries_inherent_to_i128_and_to_bool() {
        let bool_v: Value = true.into();
        let int_v: Value = 42_i64.into();
        // Named `Value::` path, matching the call-site UFCS syntax.
        // If either inherent method disappears, the trait's absence
        // becomes a compile error here first, not at the call sites.
        assert_eq!(Value::to_bool(&bool_v), Some(true));
        assert_eq!(Value::to_i128(&int_v), Some(42));
    }
}
