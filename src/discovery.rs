...
impl Format {
    ...
    /// [`Self::ordinal`] on the same closed five-cell surface the
    /// (`as_str`, canonical-label matcher) pair traverses the scalar-
    /// `&'static str` projection. Neither projection is total on the
    /// codomain — the ordinal surface admits `usize` values `>= 5`, the
    /// string surface admits non-canonical labels — so the invertor
    /// returns [`Option<Self>`] rather than a total `Self`, keeping the
    /// "not on the variant surface" case a typed [`None`] rather than
    /// a fabricated variant.
    ///
    /// Sibling landing of the const-fn ordinal-inverse peer idiom on
    /// the file-format axis: [`crate::source::FigmentSourceKind::from_ordinal`]
    /// (commit `0d979fa`) carries it on the figment-Source-axis kind,
    ...
    /// shikumi-side layer-kind axis, [`crate::tiered::DiffLineKind::from_ordinal`]
    /// (commit `7a047c9`) on the diff-cell kind axis, and
    /// [`crate::cli::OutputFormat::from_ordinal`] (commit `9ba426f`)
    /// on the CLI-side emission-format axis. Same closed-match shape,
    /// same [`Option<Self>`] return, same `const`-callability contract,
    /// applied here to the five-cell file-format axis.
    ///
    /// Load-bearing for a future
    /// [`FormatCoordinates::from_ordinal`] landing on the sibling
    /// product-cube [`FormatCoordinates`]: the algebraic two-axis
    /// inversion `AttributionSourceKindCoordinates::from_ordinal`
    /// (commit `e0b1e1e`) already ships composes the sibling axis
    /// `from_ordinal`s via integer-division / remainder on the inner-
    /// axis cardinality; without a `Format::from_ordinal` on one leg
    /// of the (`Format × FormatProvenance`) cube, the cube-side
    /// inversion cannot be spelled `const` and cannot degrade
    /// gracefully on out-of-range cells. This landing carries the
    /// outer-axis half of that construction.
    ///
    /// **Round-trip law** —
    /// `Format::from_ordinal(v.ordinal()) == Some(v)` for every
    /// `v: Format`. Composes with [`Self::ordinal`] on the same five-
    /// cell declaration order both projections match against; the law
    /// holds by construction. Pinned by
    /// [`tests::format_from_ordinal_round_trips_via_ordinal`].
    ///
    /// **Out-of-range rejection** —
    /// `Format::from_ordinal(o) == None` for every `o >= 5`. The
    ...
    /// skewed peer (a hypothetical `Cue` sixth kind in lockstep with a
    /// future `Format::Cue`) or an operator-typed CLI argument routed
    /// through [`str::parse::<usize>`][str::parse] without a bounds
    /// check — the caller reads the unknown as a typed `None` rather
    /// than a fabricated variant. Pinned by
    /// [`tests::format_from_ordinal_rejects_out_of_range`].
    ///
    /// **Const-callability** — the projection is `const fn`, matching
    /// the `const`-ness of [`Self::ordinal`] on the forward side and
    /// of [`Self::as_str`] / [`Self::provenance`] / [`Self::extensions`]
    /// on the sibling scalar surfaces. Consumers wanting a compile-
    /// time-selected ordinal-keyed dispatch (a `const [Format; 5]`
    /// variant array indexed by ordinal, a `const` per-format label
    /// built by pairing `Self::from_ordinal(o).unwrap().as_str()` at
    /// const-eval time, a per-format attestation-manifest slot in a
    /// `const` initializer keyed by ordinal) route through the
    /// projection under `const` without dropping through a runtime
    /// `let` binding. Pinned by
    /// [`tests::format_from_ordinal_is_const_callable`].
    ///
    /// **Agreement with `Self::ALL` at every index** —
    /// `Format::from_ordinal(i) == Some(Self::ALL[i])` for every
    /// `i < 5`. The inherent match and the [`Self::ALL`] slice literal
    /// carry the same declaration order (`Yaml → Toml → Lisp → Nix →
    /// Blue`) — so a future edit shifting one without the other fails
    /// at test time on the first drifted position. Pinned by
    /// [`tests::format_from_ordinal_agrees_with_all_index_pointwise`].
    #[must_use]
    ...
    /// Returns [`Some(variant)`][Some] on the exact five canonical
    /// kebab-case labels [`Self::as_str`] emits and [`None`] for every
    /// other `&str` — including the alias extensions `"yml"`/`"lsp"`/`"el"`
    /// the trait-uniform [`std::str::FromStr`] impl accepts through
    /// [`Self::from_extension`], mixed-case spellings, leading/trailing
    /// whitespace, and near-miss labels from adjacent primitives on the
    /// sealed fold.
    ///
    /// The bounded five-cell byte-match delivers:
    ///
    /// - `b"yaml"` → [`Some`]`(`[`Self::Yaml`]`)`
    /// - `b"toml"` → [`Some`]`(`[`Self::Toml`]`)`
    /// - `b"lisp"` → [`Some`]`(`[`Self::Lisp`]`)`
    /// - `b"nix"`  → [`Some`]`(`[`Self::Nix`]`)`
    /// - `b"b"`    → [`Some`]`(`[`Self::Blue`]`)`
    /// - `_`       → [`None`]
    ///
    ...
    /// projection [`Self::as_str`] on the same closed five-cell surface
    /// the (`ordinal`, `from_ordinal`) pair inverts the scalar-[`usize`]
    /// projection [`Self::ordinal`]. Neither projection is total on the
    /// codomain — the string surface admits non-canonical labels and the
    /// alias extensions, the ordinal surface admits `usize` values `>= 5`
    /// — so both invertors return [`Option<Self>`] rather than a total
    /// `Self`, keeping the "not on the canonical variant surface" case a
    /// typed [`None`] rather than a fabricated variant. With this landing
    /// the file-format axis carries the (`forward`, `inverse`) round-trip
    /// pair on BOTH closed scalar surfaces simultaneously AS A
    /// CONST-CALLABLE INHERENT.
    ///
    /// Sibling landing of the const-fn label-inverse peer idiom on the
    /// file-format axis: [`FormatProvenance::from_str`] (commit `db16832`)
    /// as the sibling inner-axis on the same (`Format × FormatProvenance`)
    /// product cube; [`crate::source::FigmentSourceKind::from_str`]
    /// (commit `140a5ff`) on the figment-Source-axis kind,
    /// [`crate::source::FigmentNameTagKind::from_str`] (commit `959a2f5`)
    /// on the figment-Name-axis kind,
    /// [`crate::source::EnvMetadataTagKind::from_str`] (commit `d9ace52`)
    /// on the env-name sub-axis kind,
    /// [`crate::source::ConfigSourceKind::from_str`] (commit `284790b`)
    /// on the shikumi-side layer-kind axis,
    /// [`crate::tiered::DiffLineKind::from_str`] (commit `7f2cdb7`) on
    /// the diff-cell kind axis, and
    /// [`crate::cli::OutputFormat::from_str`] (commit `9f3631f`) on the
    /// CLI-side emission-format axis. Same closed-match shape, same
    /// [`Option<Self>`] return, same `const`-callability contract,
    /// applied here to the five-cell file-format axis, matching the
    /// sibling const-fn ordinal-inverse peer [`Self::from_ordinal`]
    /// (commit `961c830`) one scalar altitude over on the same primitive.
    ///
    /// With this landing BOTH legs of the (`Format × FormatProvenance`)
    /// product cube — outer `Format::from_str` (this commit) and inner
    /// [`FormatProvenance::from_str`] (commit `db16832`) — are
    /// `const`-callable, so a future [`FormatCoordinates::from_str`]
    /// inherent (const, [`Option<Self>`]) becomes expressible without
    /// dropping through the runtime `let` binding the current
    /// `<FormatCoordinates as FromStr>::from_str` requires: composing the
    /// two sibling const-fn label inverses under a `split_once(':')` and
    /// returning [`None`] on any of the three failure paths (missing
    /// separator, unknown format-half label, unknown provenance-half
    /// label). The scalar-ordinal peer construction already ships with
    /// both legs const-invertible via [`Self::from_ordinal`] +
    /// [`FormatProvenance::from_ordinal`] (commit `c468a86`).
    ///
    ...
    /// byte-form [`Self::as_str`] emits, keeping the projection
    /// const-callable and the five-cell inverse a total function on the
    /// canonical codomain without dragging in the trait's
    /// case-insensitive branch. Peer of the macro-generated
    /// [`std::str::FromStr`] impl (via
    /// [`crate::closed_axis_label_string_surface!`]) one seam over that
    /// delegates to [`Self::from_extension`] and returns
    /// [`Result<Self, crate::ShikumiError>`] with a formatted parse-error
    /// legend — accepting `"yml"`/`"lsp"`/`"el"` alongside the canonical
    /// labels and lowering mixed case at every call site; this inherent
    /// instead returns [`Option<Self>`], keeping the "not on the exact
    /// canonical variant surface" case a typed [`None`] without
    /// allocating an error string and without accepting the alias
    /// extensions. Downstream call sites that previously reached the
    /// trait impl through unqualified `Format::from_str(...)` disambiguate
    /// through `<Format as FromStr>::from_str(...)` (e.g. the
    /// [`FormatCoordinates`] `FromStr` implementation's format-half
    /// lowering, which retains alias-extension acceptance through the
    /// trait method).
    ///
    ...
    /// [`Self::from_ordinal`] and of every sibling const-fn label
    /// inverse. A consumer wanting case-insensitive parsing (an
    /// operator-typed `--format YAML` at a CLI, a mixed-case tag in a
    /// Markdown-rendered chain summary) reaches for the trait-uniform
    /// [`<Self as crate::ClosedAxisLabel>::from_canonical_str`], the
    /// macro-generated `FromStr` impl (which lowers through
    /// [`Self::from_extension`], case-insensitive), or lowercases at
    /// their own site.
    ///
    /// **Alias rejection** — the aliases `"yml"`, `"lsp"`, and `"el"`
    /// that [`Self::from_extension`] accepts are NOT on the canonical
    /// codomain [`Self::as_str`] emits and resolve to [`None`] here. A
    /// consumer wanting alias-extension acceptance reaches for the
    /// trait-uniform `<Format as FromStr>` (which delegates to
    /// [`Self::from_extension`]) or calls [`Self::from_extension`]
    /// directly. This inherent is the const-callable, canonical-only
    /// label inverse; the alias algebra lives one seam over on the
    /// trait-uniform surface.
    ///
    /// **Round-trip law** —
    /// `Format::from_str(v.as_str()) == Some(v)` for every `v: Format`.
    /// Composes with [`Self::as_str`] on the same five-cell label table
    /// both projections match against; the law holds by construction.
    /// Pinned by [`tests::format_from_str_round_trips_via_as_str`].
    ///
    /// **Non-canonical rejection** —
    /// `Format::from_str(s) == None` for every `s` outside the canonical
    /// five-cell set `{"yaml", "toml", "lisp", "nix", "b"}`. The closed
    /// match's `_` arm forwards the off-surface case to [`None`]
    /// structurally; the guard degrades gracefully on a version-skewed
    /// peer emitting a stale label, a mixed-case operator-typed CLI
    /// argument that never went through a lowering step, an alias
    /// extension (`"yml"`/`"lsp"`/`"el"`), or the empty string. Pinned
    /// by [`tests::format_from_str_rejects_non_canonical`].
    ///
    /// **Const-callability** — the projection is `const fn`, matching
    /// the `const`-ness of [`Self::as_str`] on the forward side and of
    /// [`Self::ordinal`] / [`Self::from_ordinal`] on the sibling
    /// scalar-surface pair. Consumers wanting a compile-time-selected
    /// label-keyed dispatch (a `const [Format; 5]` variant array
    /// recovered from a `const &[&str; 5]` canonical-label list, a
    /// per-format attestation-manifest slot in a `const` initializer
    /// keyed by canonical label) route through the projection under
    /// `const` without dropping through a runtime `let` binding. Pinned
    /// by [`tests::format_from_str_is_const_callable`].
    ///
    /// **Agreement with [`Self::as_str`] pointwise** —
    /// `Format::from_str(v.as_str()) == Some(v)` for every `v: Format`.
    /// The inherent match and the forward [`Self::as_str`] match carry
    /// the same five-cell label mapping; the test below pins the
    /// pointwise agreement across every variant, and a future edit that
    /// shifts the label on ONE match without the other fails at test
    /// time on the first drifted arm. Pinned by
    /// [`tests::format_from_str_agrees_with_as_str_pointwise`].
    ///
    /// **Agreement with [`crate::ClosedAxisLabel::from_canonical_str`]
    /// on canonical input** — for every `v: Format`,
    /// `Format::from_str(v.as_str()) ==
    /// <Format as ClosedAxisLabel>::from_canonical_str(v.as_str())`.
    ...
    /// [`tests::format_from_str_agrees_with_closed_axis_label_from_canonical_str_on_canonical_input`].
    #[must_use]
...
impl FormatProvenance {
    ...
    /// Const-fn canonical-label → variant inverse of [`Self::as_str`].
    /// Returns [`Some(variant)`][Some] for every canonical kebab-case
    /// label — the exact two-cell codomain [`Self::as_str`] emits — and
    /// [`None`] for any other `&str`.
    ///
    /// The bounded two-cell match delivers:
    ///
    /// - `"figment-builtin"` → [`Some`]`(`[`Self::FigmentBuiltin`]`)`
    /// - `"shikumi-built"`   → [`Some`]`(`[`Self::ShikumiBuilt`]`)`
    /// - `_`                 → [`None`]
    ///
    /// Sibling of [`Self::from_ordinal`] on the scalar-[`usize`] surface:
    /// the (`as_str`, `from_str`) pair inverts the scalar-[`&'static str`]
    ...
    /// "not on the variant surface" case a typed [`None`] rather than a
    /// fabricated variant. With this landing the provenance axis carries
    /// the (`forward`, `inverse`) round-trip pair on BOTH closed scalar
    /// surfaces simultaneously AS A CONST-CALLABLE INHERENT.
    ///
    /// Sibling landing of the const-fn label-inverse peer idiom on the
    /// provenance axis: [`crate::source::FigmentSourceKind::from_str`]
    /// (commit `140a5ff`) on the figment-Source-axis kind,
    /// [`crate::source::FigmentNameTagKind::from_str`] (commit `959a2f5`)
    /// on the figment-Name-axis kind,
    /// [`crate::source::EnvMetadataTagKind::from_str`] (commit `d9ace52`)
    ...
    /// CLI-side emission-format axis. Same closed-match shape, same
    /// [`Option<Self>`] return, same `const`-callability contract,
    /// applied here to the two-cell provenance axis, matching the
    /// sibling const-fn ordinal-inverse peer [`Self::from_ordinal`]
    /// (commit `c468a86`) one scalar altitude over on the same
    /// primitive.
    ///
    /// Peer of the trait-uniform
    /// [`<Self as crate::ClosedAxisLabel>::from_canonical_str`] one seam
    ...
    /// canonical byte-form [`Self::as_str`] emits, keeping the projection
    /// const-callable and the two-cell inverse a total function on the
    /// canonical codomain without dragging in the trait's
    /// case-insensitive branch. Peer of the macro-generated
    /// [`std::str::FromStr`] impl (via
    /// [`crate::closed_axis_label_string_surface!`]) one seam over that
    ...
    /// [`None`] without allocating an error string. Downstream call
    /// sites that previously reached the trait impl through
    /// unqualified `FormatProvenance::from_str(...)` now disambiguate
    /// through `<FormatProvenance as FromStr>::from_str(...)` (e.g. the
    /// [`FormatCoordinates`] `FromStr` implementation's provenance-half
    /// lowering).
    ///
    /// **Case sensitivity** — the match is exact-byte on the canonical
    ...
    /// [`Self::from_ordinal`] and of every sibling const-fn label
    /// inverse. A consumer wanting case-insensitive parsing (an
    /// operator-typed `--provenance FIGMENT-BUILTIN` at a CLI, a
    /// mixed-case tag in a Markdown-rendered chain summary) reaches
    /// for the trait-uniform
    /// [`<Self as crate::ClosedAxisLabel>::from_canonical_str`] or
    /// lowercases at their own site.
    ///
    /// **Round-trip law** —
    /// `FormatProvenance::from_str(v.as_str()) == Some(v)` for every
    /// `v: FormatProvenance`. Composes with [`Self::as_str`] on the same
    /// two-cell label table both projections match against; the law
    /// holds by construction. Pinned by
    /// [`tests::format_provenance_from_str_round_trips_via_as_str`].
    ///
    /// **Non-canonical rejection** —
    /// `FormatProvenance::from_str(s) == None` for every `s` outside the
    /// canonical two-cell set `{"figment-builtin", "shikumi-built"}`.
    /// The closed match's `_` arm forwards the off-surface case to
    /// [`None`] structurally; the guard degrades gracefully on a
    /// version-skewed peer emitting a stale label, a mixed-case
    /// operator-typed CLI argument that never went through a lowering
    /// step, or the empty string. Pinned by
    /// [`tests::format_provenance_from_str_rejects_non_canonical`].
    ///
    /// **Const-callability** — the projection is `const fn`, matching
    /// the `const`-ness of [`Self::as_str`] on the forward side and of
    /// [`Self::ordinal`] / [`Self::from_ordinal`] on the sibling
    /// scalar-surface pair. Consumers wanting a compile-time-selected
    /// label-keyed dispatch (a `const [FormatProvenance; 2]` variant
    /// array recovered from a `const &[&str; 2]` canonical-label list,
    /// a per-provenance attestation-manifest slot in a `const`
    /// initializer keyed by canonical label) route through the
    /// projection under `const` without dropping through a runtime
    /// `let` binding. Pinned by
    /// [`tests::format_provenance_from_str_is_const_callable`].
    ///
    /// **Agreement with [`Self::as_str`] pointwise** —
    /// `FormatProvenance::from_str(v.as_str()) == Some(v)` for every
    /// `v: FormatProvenance`. The inherent match and the forward
    ...
    /// [`tests::format_provenance_from_str_agrees_with_as_str_pointwise`].
    ///
    /// **Agreement with [`crate::ClosedAxisLabel::from_canonical_str`]
    /// on canonical input** — for every `v: FormatProvenance`,
    /// `FormatProvenance::from_str(v.as_str()) ==
    /// <FormatProvenance as ClosedAxisLabel>::from_canonical_str(v.as_str())`.
    /// Both seams recover the same variant on the canonical kebab-case
    /// codomain; they diverge only OFF that codomain (the trait method
    /// case-insensitive-lowers, the inherent rejects). Pinned by
    /// [`tests::format_provenance_from_str_agrees_with_closed_axis_label_from_canonical_str_on_canonical_input`].
    #[must_use]
    #[allow(clippy::should_implement_trait)]
    pub const fn from_str(s: &str) -> Option<Self> {
        match s.as_bytes() {
            b"figment-builtin" => Some(Self::FigmentBuiltin),
            b"shikumi-built" => Some(Self::ShikumiBuilt),
            _ => None,
        }
    }
...
impl FromStr for FormatCoordinates {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let format = <Format as FromStr>::from_str(format_half).map_err(|_| {
            ParseFormatCoordinatesError::UnknownFormat {
                label: format_half.to_owned(),
            }
        })?;
        let provenance =
            <FormatProvenance as FromStr>::from_str(provenance_half).map_err(|_| {
...
mod tests {
    ...
    #[test]
    fn format_from_ordinal_round_trips_via_ordinal() {
        // Round-trip law: `Format::from_ordinal(v.ordinal()) == Some(v)`
        // for every v: Format. The forward-map `ordinal` and the inverse-
        // map `from_ordinal` share the SAME closed five-cell declaration
        // order (`Format::ALL`, `Yaml → Toml → Lisp → Nix → Blue`); the
        ...
        // the figment-Name-axis kind (commit `6b8dccf`),
        // `env_metadata_tag_kind_from_ordinal_round_trips_via_ordinal` on
        // the env-name sub-axis kind (commit `72c523a`), and
        // `config_source_kind_from_ordinal_round_trips_via_ordinal` on
        // the shikumi-side layer-kind axis.
        for &format in Format::ALL {
            let ordinal = format.ordinal();
            let recovered = Format::from_ordinal(ordinal);
            assert_eq!(
                recovered,
                Some(format),
                "round-trip failed for {format:?}: ordinal={ordinal} did not parse back",
            );
        }
    }

    #[test]
    fn format_from_ordinal_rejects_out_of_range() {
        ...
        // ordinal from a version-skewed peer (a hypothetical `Cue` sixth
        // kind in lockstep with a future `Format::Cue`) or an operator-
        // typed CLI argument routed through `str::parse::<usize>` without
        // a bounds check — the caller reads the unknown as a typed `None`
        // rather than a fabricated variant. Sibling of
        // `figment_source_kind_from_ordinal_rejects_out_of_range` on the
        // figment-Source-axis kind.
        assert_eq!(Format::from_ordinal(5), None);
        assert_eq!(Format::from_ordinal(6), None);
        assert_eq!(Format::from_ordinal(42), None);
        assert_eq!(Format::from_ordinal(usize::MAX), None);
    }

    #[test]
    fn format_from_ordinal_agrees_with_all_index_pointwise() {
        // `from_ordinal(i) == Some(Format::ALL[i])` for every i in 0..5 —
        // the inverse of `ordinal` agrees with the same `Self::ALL` slice
        // literal `ordinal` matches against. A future edit shifting one
        // match without the other fails here on the first drifted index.
        // Sibling of
        // `figment_source_kind_from_ordinal_agrees_with_all_index_pointwise`
        // on the figment-Source-axis kind.
        for (index, &expected) in Format::ALL.iter().enumerate() {
            assert_eq!(
                Format::from_ordinal(index),
                Some(expected),
                "from_ordinal({index}) must agree with Format::ALL[{index}]",
            );
        }
        // Beyond the axis cardinality (5) the projection returns None at
        // every offset. Pin the immediate boundary to catch a future
        // off-by-one landing on the first out-of-range slot.
        assert_eq!(
            Format::from_ordinal(Format::ALL.len()),
            None,
            "ordinal equal to Format::ALL.len() must be out of range",
        );
    }

    #[test]
    fn format_from_ordinal_agrees_with_axis_at_pointwise() {
        // Cross-seam agreement: the inherent const-fn `Format::from_ordinal`
        // and the trait-uniform free-function projection
        // `crate::axis_at::<Format>` are two spellings of the same closed-
        // axis position → variant lookup; pin them pointwise across every
        // in-range and out-of-range ordinal so a future edit to either
        // the inherent match or the `Format::ALL` declaration order
        // cannot silently drift them apart. The inherent seam ships
        ...
        // `[0, Format::ALL.len() + 32)` plus `usize::MAX` as the
        // arithmetic extreme — verifies the closed match's `_` arm
        // agrees with the `axis_at` `None` on every out-of-range index
        // the projection can plausibly receive.
        for ordinal in 0..(Format::ALL.len() + 32) {
            assert_eq!(
                Format::from_ordinal(ordinal),
                crate::axis_at::<Format>(ordinal),
                "from_ordinal must agree with axis_at at ordinal={ordinal}",
            );
        }
        assert_eq!(
            Format::from_ordinal(usize::MAX),
            crate::axis_at::<Format>(usize::MAX),
            "from_ordinal must agree with axis_at at usize::MAX",
        );
    }

    #[test]
    fn format_from_ordinal_is_const_callable() {
        // Compile-time weld: the (ordinal → variant) inverse is
        // `const`-callable, matching the `const`-ness of the forward
        // projection `Format::ordinal` and the sibling `Format::as_str` /
        // `Format::provenance` / `Format::extensions`. A drop of the
        // `const` qualifier on `Format::from_ordinal` fails this test to
        // compile.
        //
        ...
        // `figment_source_kind_from_ordinal_is_const_callable` on the
        // figment-Source-axis kind.
        const AT_0: Option<Format> = Format::from_ordinal(0);
        const AT_1: Option<Format> = Format::from_ordinal(1);
        const AT_2: Option<Format> = Format::from_ordinal(2);
        const AT_3: Option<Format> = Format::from_ordinal(3);
        const AT_4: Option<Format> = Format::from_ordinal(4);
        const AT_5: Option<Format> = Format::from_ordinal(5);

        assert_eq!(AT_0, Some(Format::Yaml));
        assert_eq!(AT_1, Some(Format::Toml));
        assert_eq!(AT_2, Some(Format::Lisp));
        assert_eq!(AT_3, Some(Format::Nix));
        assert_eq!(AT_4, Some(Format::Blue));
        assert_eq!(AT_5, None);
    }

    #[test]
    fn format_from_str_round_trips_via_as_str() {
        // Round-trip law: `Format::from_str(v.as_str()) == Some(v)` for
        // every v: Format. The forward-map `as_str` and the const-fn
        // inverse-map `from_str` share the SAME closed five-cell label
        // table (`"yaml"`, `"toml"`, `"lisp"`, `"nix"`, `"b"` in
        // Format::ALL declaration order); the law holds by construction.
        // Sibling of
        // `format_provenance_from_str_round_trips_via_as_str` on the
        // provenance axis (commit `db16832`) — the sibling inner-axis on
        // the same (`Format × FormatProvenance`) product cube — and every
        // other `_from_str_round_trips_via_as_str` seal on the sibling
        // closed-axis primitives; scalar-altitude peer of
        // `format_from_ordinal_round_trips_via_ordinal` on the scalar-
        // usize surface of the same primitive.
        for &format in Format::ALL {
            let rendered = format.as_str();
            let recovered = Format::from_str(rendered);
            assert_eq!(
                recovered,
                Some(format),
                "round-trip failed for {format:?}: as_str={rendered:?} did not parse back",
            );
        }
    }

    #[test]
    fn format_from_str_rejects_non_canonical() {
        // Non-canonical rejection: any `&str` outside the exact canonical
        // five-cell set `{"yaml", "toml", "lisp", "nix", "b"}` — the
        // codomain of `as_str` — resolves to `None` via the closed
        // match's `_` arm. Sweeps mixed-case (`"YAML"`, `"Toml"`,
        // `"LiSp"`), leading/trailing whitespace, the empty string, the
        // alias extensions `"yml"` / `"lsp"` / `"el"` the trait-uniform
        // `FromStr` accepts through `from_extension` (rejected here — the
        // inherent is canonical-only), the sibling `FormatProvenance`
        // labels one axis over on the (`Format × FormatProvenance`)
        // product cube (`"figment-builtin"` / `"shikumi-built"`, valid
        // there, rejected here), and near-miss spellings from adjacent
        // primitives on the sealed fold (`"json"` — a hypothetical future
        // Format extension; `"defaults"` / `"file"` / `"env"` — sibling
        // ConfigSourceKind labels one primitive over). The
        // case-sensitivity discipline matches the sibling
        // `FormatProvenance::from_str` / `EnvMetadataTagKind::from_str` /
        // `ConfigSourceKind::from_str` const-fn label inverses; callers
        // wanting case-insensitive parsing or alias acceptance reach for
        // the trait-uniform `<Self as ClosedAxisLabel>::from_canonical_str`
        // or the macro-generated `FromStr` impl (which delegates to
        // `from_extension`, ASCII case-insensitive over
        // canonical + aliases).
        for bad in &[
            "YAML",
            "Yaml",
    ...
    #[test]
    fn format_from_str_is_const_callable() {
        // Compile-time weld: the (label → variant) inverse is
        // `const`-callable, matching the `const`-ness of the forward
        // projection `Format::as_str` and the sibling `Format::from_ordinal`.
        // A drop of the `const` qualifier on `Format::from_str` fails
        ...
        // before the drift can reach downstream consumers that assumed
        // const-ness through the projection. Sibling of
        // `format_provenance_from_str_is_const_callable` on the sibling
        // inner-axis of the same (`Format × FormatProvenance`) product
        // cube and of `format_from_ordinal_is_const_callable` on the
        // scalar-usize surface of the same primitive.
        const AT_YAML: Option<Format> = Format::from_str("yaml");
        const AT_TOML: Option<Format> = Format::from_str("toml");
        const AT_LISP: Option<Format> = Format::from_str("lisp");
        const AT_NIX: Option<Format> = Format::from_str("nix");
        const AT_BLUE: Option<Format> = Format::from_str("b");
        const AT_UNKNOWN: Option<Format> = Format::from_str("yml");

        assert_eq!(AT_YAML, Some(Format::Yaml));
        assert_eq!(AT_TOML, Some(Format::Toml));
        assert_eq!(AT_LISP, Some(Format::Lisp));
        assert_eq!(AT_NIX, Some(Format::Nix));
        assert_eq!(AT_BLUE, Some(Format::Blue));
        assert_eq!(AT_UNKNOWN, None);
    }

    #[test]
    fn format_from_str_agrees_with_as_str_pointwise() {
        // Pointwise agreement: `from_str(v.as_str()) == Some(v)` for
        // every v in Format::ALL. Both the inherent `from_str` match and
        // the forward `as_str` match derive their label table from the
        // same five-cell declaration; a future edit that shifts the label
        // on ONE match (say renaming the `Blue` arm's label to `"blue"`
        // on the `as_str` side but not the `from_str` side, or vice
        // versa) fails here on the first drifted arm. Sibling of
        // `format_provenance_from_str_agrees_with_as_str_pointwise` on
        // the sibling inner-axis of the same
        // (`Format × FormatProvenance`) product cube.
        for &format in Format::ALL {
            assert_eq!(
                Format::from_str(format.as_str()),
                Some(format),
                "from_str(as_str) must agree pointwise for {format:?}",
            );
        }
    }

    #[test]
    fn format_from_str_agrees_with_closed_axis_label_from_canonical_str_on_canonical_input() {
        ...
        // label seam on the closed variant surface. Sibling of
        // `format_provenance_from_str_agrees_with_closed_axis_label_from_canonical_str_on_canonical_input`
        // on the sibling inner-axis of the same
        // (`Format × FormatProvenance`) product cube.
        use crate::ClosedAxisLabel;
        for &format in Format::ALL {
            let label = format.as_str();
            let inherent = Format::from_str(label);
            let trait_uniform = <Format as ClosedAxisLabel>::from_canonical_str(label);
            assert_eq!(
    ...
    #[test]
    fn format_provenance_from_str_round_trips_via_as_str() {
        // Round-trip law: `FormatProvenance::from_str(v.as_str()) ==
        // Some(v)` for every v: FormatProvenance. The forward-map
        // `as_str` and the const-fn inverse-map `from_str` share the
        // SAME closed two-cell label table (`"figment-builtin"`,
        // `"shikumi-built"` in FormatProvenance::ALL declaration order);
        // the law holds by construction. Sibling of
        // `env_metadata_tag_kind_from_str_round_trips_via_as_str` on the
        // env-name sub-axis kind (commit `d9ace52`) and every other
        // `_from_str_round_trips_via_as_str` seal on the sibling
        // closed-axis primitives; scalar-altitude peer of
        // `format_provenance_from_ordinal_round_trips_via_ordinal` on
        // the scalar-usize surface of the same primitive.
        for &provenance in FormatProvenance::ALL {
            let rendered = provenance.as_str();
            let recovered = FormatProvenance::from_str(rendered);
            assert_eq!(
                recovered,
                Some(provenance),
                "round-trip failed for {provenance:?}: \
                 as_str={rendered:?} did not parse back",
            );
        }
    }

    #[test]
    fn format_provenance_from_str_rejects_non_canonical() {
        // Non-canonical rejection: any `&str` outside the exact
        // canonical two-cell set `{"figment-builtin", "shikumi-built"}`
        // — the codomain of `as_str` — resolves to `None` via the
        // closed match's `_` arm. Sweeps mixed-case
        // (`"FIGMENT-BUILTIN"`, `"Shikumi-Built"`,
        // `"fIgMeNt-BuIlTiN"`), leading/trailing whitespace, the empty
        // string, adjacent labels a hypothetical future extension might
        // introduce (`"custom"`, `"upstream-figment-ecosystem"`), and
        // near-miss spellings from adjacent primitives on the sealed
        // fold (`"yaml"` / `"toml"` — sibling Format labels one primitive
        // over, valid there, rejected here; `"defaults"` / `"file"` /
        // `"env"` — sibling ConfigSourceKind labels one axis over on
        // the (tier, source) pair, valid there, rejected here). The
        // case-sensitivity discipline matches the sibling
        // `EnvMetadataTagKind::from_str` / `ConfigSourceKind::from_str`
    ...
    #[test]
    fn format_provenance_from_str_is_const_callable() {
        // Compile-time weld: the (label → variant) inverse is
        // `const`-callable, matching the `const`-ness of the forward
        // projection `FormatProvenance::as_str` and the sibling
        // `FormatProvenance::from_ordinal`. A drop of the `const`
        // qualifier on `FormatProvenance::from_str` fails this test to
        // compile.
        //
        // Three `const` bindings — two in-range plus one out-of-range —
        // route each canonical label through the const-fn inverse in
        ...
        // `env_metadata_tag_kind_from_str_is_const_callable` on the
        // env-name sub-axis kind and of
        // `format_provenance_from_ordinal_is_const_callable` on the
        // scalar-usize surface of the same primitive.
        const AT_FIGMENT_BUILTIN: Option<FormatProvenance> =
            FormatProvenance::from_str("figment-builtin");
        const AT_SHIKUMI_BUILT: Option<FormatProvenance> =
            FormatProvenance::from_str("shikumi-built");
        const AT_UNKNOWN: Option<FormatProvenance> = FormatProvenance::from_str("custom");

        assert_eq!(AT_FIGMENT_BUILTIN, Some(FormatProvenance::FigmentBuiltin));
        assert_eq!(AT_SHIKUMI_BUILT, Some(FormatProvenance::ShikumiBuilt));
        assert_eq!(AT_UNKNOWN, None);
    }

    #[test]
    fn format_provenance_from_str_agrees_with_as_str_pointwise() {
        // Pointwise agreement: `from_str(v.as_str()) == Some(v)` for
        // every v in FormatProvenance::ALL. Both the inherent `from_str`
        // match and the forward `as_str` match derive their label table
        // from the same two-cell declaration; a future edit that shifts
        // the label on ONE match (say renaming the `ShikumiBuilt` arm's
        // label to `"shikumi"` on the `as_str` side but not the
        // `from_str` side, or vice versa) fails here on the first
        // drifted arm. Sibling of
        // `env_metadata_tag_kind_from_str_agrees_with_as_str_pointwise`
        // on the env-name sub-axis kind one primitive over.
        for &provenance in FormatProvenance::ALL {
            assert_eq!(
                FormatProvenance::from_str(provenance.as_str()),
                Some(provenance),
                "from_str(as_str) must agree pointwise for {provenance:?}",
            );
        }
    }

    #[test]
    fn format_provenance_from_str_agrees_with_closed_axis_label_from_canonical_str_on_canonical_input()
     {
        // Cross-seam agreement on the canonical codomain: for every
        // variant, `from_str(v.as_str()) ==
        // <Self as ClosedAxisLabel>::from_canonical_str(v.as_str())`.
        // Both seams recover the same variant on the exact kebab-case
        // labels `as_str` emits; they diverge only OFF that codomain
        // (the trait method case-insensitive-lowers non-canonical
        // labels, the inherent rejects them structurally). This pin
        // cross-checks the const-fn label seam against the trait-
        // uniform label seam on the closed variant surface. Sibling of
        // `env_metadata_tag_kind_from_str_agrees_with_closed_axis_label_from_canonical_str_on_canonical_input`
        // on the env-name sub-axis kind one primitive over.
        use crate::ClosedAxisLabel;
        for &provenance in FormatProvenance::ALL {
            let label = provenance.as_str();
            let inherent = FormatProvenance::from_str(label);
            let trait_uniform = <FormatProvenance as ClosedAxisLabel>::from_canonical_str(label);
            assert_eq!(
                inherent, trait_uniform,
                "canonical label {label:?} must recover the same variant through \
                 the inherent const-fn `from_str` and the trait-uniform \
