...
impl HintedCoverageReport {
    ...
    /// (and the enumeration behind [`Self::hint_count_by_surface`]) at
    /// the coverage sub-report scope, and the sub-report analogue of
    /// [`HealthReport::hint_iter_by_surface`].
    ///
    /// Yields exactly the hints whose surface tag matches `surface`,
    /// in the same order [`Self::hint_iter`] would yield them —
    /// dead knobs first when `surface == HintSurface::DeadKnob`, stale
    /// entries when `surface == HintSurface::StaleEntry`, and the empty
    /// iterator for the two variants this sub-report carries no hints
    /// on ([`HintSurface::ValueKey`], [`HintSurface::EnvVar`]) — the
    /// sub-report-bounded fact welded to the type: any surface not in
    /// the sub-report's own two-variant sub-partition of
    /// [`HintSurface::ALL`] yields empty here by construction, so the
    /// summed histogram across [`HintSurface::ALL`] collapses to the
    /// two own-surface counts without ever double-counting a foreign
    /// surface. Within each own surface, hints follow the order of
    /// their underlying hint list (sorted by entry per the audit
    /// primitives, so the enumeration is deterministic end-to-end).
    ///
    /// `hint_iter_by_surface(s).count()` equals
    /// [`Self::hint_count_by_surface`]`(s)` by construction, and
    /// summing across [`HintSurface::ALL`] recovers [`Self::hint_count`]
    /// with NO cross-surface dedup — the same fold-composition
    /// invariant [`Self::hint_count`] promises across the sub-report's
    /// two hint lists, projected here onto the four hint surfaces (the
    /// two foreign surfaces contributing zero terms).
    ///
    /// Delegates through [`Self::hint_iter`] itself — a
    /// `.filter(|h| h.surface == surface)` — so the two enumerations
    /// cannot drift, and the peer folds up to
    /// [`HealthReport::hint_iter_by_surface`] on its own-surface
    /// prefix: for `s ∈ {HintSurface::DeadKnob, HintSurface::StaleEntry}`,
    /// `hinted.hint_iter_by_surface(s)` yields the same sequence as
    /// `health.hint_iter_by_surface(s)` on shared inputs, since the
    /// whole-report peer's coverage-arm prefix IS this sub-report's
    /// own enumeration (welded by
    /// `hinted_coverage_hint_iter_matches_health_report_coverage_prefix`
    /// one axis up on the un-filtered pair).
    ///
    /// Consumers holding only a [`HintedCoverageReport`] (a `#[test]`
    /// that only runs [`ConfigCoverage::hinted_report`] without a full
    /// [`ConfigCoverage::health_report`], a coverage-only dashboard
    /// tile that separates dead-knob and stale-entry columns, a
    /// per-surface JSON row array on a coverage-only diagnostics
    /// endpoint) call this directly instead of
    /// `hint_iter().filter(...)` — the named peer says "this is the
    /// supported access pattern at the sub-report scope" and pairs
    /// with [`Self::hint_count_by_surface`] on the count side.
    ...
    /// Count of hints on ONE [`HintSurface`] at the coverage sub-report
    /// scope — the surface-projected peer of [`Self::hint_count`] (and
    /// the count-side peer of [`Self::hint_iter_by_surface`]), the
    /// sub-report analogue of [`HealthReport::hint_count_by_surface`].
    ///
    ...
    /// drift. Summing across [`HintSurface::ALL`] recovers
    /// [`Self::hint_count`] with NO cross-surface dedup — the same
    /// fold-composition invariant [`Self::hint_count`] promises across
    /// the sub-report's two hint lists, projected here onto the four
    /// hint surfaces (the two foreign-to-this-sub-report surfaces
    /// [`HintSurface::ValueKey`] and [`HintSurface::EnvVar`]
    /// contributing zero terms by construction).
    ///
    /// For `s ∈ {HintSurface::DeadKnob, HintSurface::StaleEntry}`
    /// this equals [`HealthReport::hint_count_by_surface`]`(s)` on
    /// shared inputs — the two sub-report own-surface counts are the
    /// whole-report own-surface counts, since the whole-report count
    /// folds through the three sub-report enumerations and only the
    /// coverage sub-report contributes to those two surfaces.
    ///
    /// Consumers that want to prioritise remediation on ONE own
    /// surface at the sub-report scope ("only page on dead knobs, not
    /// stale entries"), gate CI on a per-surface hint-count budget on
    /// this sub-report alone, or feed a per-surface severity number to
    /// a coverage-only diagnostics endpoint call this directly instead
    /// of hand-walking [`Self::dead_knobs`] / [`Self::stale_entries`].
    #[must_use]
    ...
    /// The plural batch peer of [`Self::hint_count_by_surface`] and the
    /// count-side histogram of [`Self::hint_iter`] at the coverage
    /// sub-report scope — every [`HintSurface`] variant paired with its
    /// per-surface hint count on this sub-report, folded through
    /// [`HintSurface::ALL`] in canonical yield order (dead knobs, stale
    /// entries, unknown value keys, unknown env vars). The sub-report
    /// analogue of [`HealthReport::hint_counts_by_surface`], and the
    /// count-side batch peer of [`Self::is_clean_by_surface`] one axis
    /// over.
    ///
    /// Returns a fixed-size
    /// `[(HintSurface, usize); HintSurface::ALL.len()]` array so the
    /// "one entry per surface, no missing surface, no duplicate
    /// surface" claim is a fact of the type — no runtime length check,
    /// no room for a caller to see a partial histogram, no room for a
    /// `BTreeMap`-shaped alternative to silently drop a zero-count
    /// surface (a variant added to [`HintSurface`] with its
    ...
    /// sum of the array's counts equals [`Self::hint_count`] with NO
    /// cross-surface dedup — the same fold-composition invariant
    /// [`Self::hint_count_by_surface`] promises pointwise, promoted
    /// here to a whole-histogram fact so a consumer that emits the
    /// full per-sub-report histogram (a Prometheus gauge vector with
    /// `surface` as the label scoped to the coverage sub-report, a
    /// dashboard tile row with one per-surface count for this
    /// sub-report alone, a golden fixture that pins the sub-report's
    /// per-surface counts across clean and dirty runs) has ONE call
    /// site instead of a hand-walked loop over [`HintSurface::ALL`].
    ///
    /// The two foreign-to-this-sub-report entries
    /// ([`HintSurface::ValueKey`], [`HintSurface::EnvVar`]) are
    /// ALWAYS `0` here — the sub-report-bounded fact
    /// [`Self::hint_count_by_surface`] promises pointwise on those
    /// two variants, promoted to a whole-histogram fact and welded
    /// by
    /// `hinted_coverage_hint_counts_by_surface_is_zero_on_foreign_surfaces`.
    /// On the two own-surface variants ([`HintSurface::DeadKnob`],
    /// [`HintSurface::StaleEntry`]) this equals the corresponding
    /// entry of [`HealthReport::hint_counts_by_surface`] on shared
    /// inputs — the whole-report/sub-report compositional wiring
    /// welded pointwise by
    /// `hinted_coverage_hint_counts_by_surface_matches_health_report_on_own_surfaces`,
    /// the count-side plural-batch peer of the identity
    /// [`Self::hint_count_by_surface`] promises on the scalar per-
    /// surface layer.
    ///
    /// Delegates through [`Self::hint_count_by_surface`] pointwise so
    /// this whole-histogram peer cannot drift from its scalar per-
    /// surface peer, and cannot drift from the enumeration primitive
    /// [`Self::hint_iter`] the scalar peer delegates through in turn.
    #[must_use]
    ...
    /// bool-side histogram of [`Self::hint_iter`] at the coverage
    /// sub-report scope — every [`HintSurface`] variant paired with its
    /// per-surface cleanness predicate on this sub-report, folded through
    /// [`HintSurface::ALL`] in canonical yield order (dead knobs, stale
    /// entries, unknown value keys, unknown env vars). The sub-report
    /// analogue of [`HealthReport::is_clean_by_surfaces`], and the
    /// bool-side batch peer of [`Self::hint_counts_by_surface`] on the
    /// count-side axis over.
    ///
    /// Returns a fixed-size
    ...
    /// surface" claim is a fact of the type — no runtime length check,
    /// no room for a caller to see a partial histogram, no room for a
    /// `BTreeMap`-shaped alternative to silently drop a clean surface
    /// (a variant added to [`HintSurface`] with its
    /// [`HintSurface::ALL`] entry — welded by
    /// `hint_surface_all_covers_every_variant_exactly_once` —
    /// automatically grows the returned array's cardinality, and every
    /// consumer picks up the new surface without a code change).
    ...
    /// consumer that emits the full per-sub-report cleanness table (a
    /// Prometheus gauge vector with `surface` as the label and a `0`/`1`
    /// value scoped to the coverage sub-report, a coverage-only
    /// dashboard tile row with one traffic-light per surface, a
    /// coverage-only golden fixture that pins the sub-report's
    /// per-surface cleanness table across clean and dirty runs) has ONE
    /// call site instead of a hand-walked loop over
    /// [`HintSurface::ALL`].
    ///
    /// The two foreign-to-this-sub-report entries
    /// ([`HintSurface::ValueKey`], [`HintSurface::EnvVar`]) are ALWAYS
    /// `true` here — the sub-report-bounded fact
    /// [`Self::is_clean_by_surface`] promises pointwise on those two
    ...
    /// of [`HealthReport::is_clean_by_surfaces`] on shared inputs — the
    ...
    /// the bool-side plural-batch peer of the identity
    /// [`Self::is_clean_by_surface`] promises on the scalar per-surface
    /// layer.
    ///
    /// Delegates through [`Self::is_clean_by_surface`] pointwise so this
    /// whole-histogram peer cannot drift from its scalar per-surface
    /// peer, cannot drift from the count-side sibling
    /// [`Self::hint_counts_by_surface`] on the same axis, and cannot
    /// drift from the enumeration primitive [`Self::hint_iter`] beneath
    /// all three.
    #[must_use]
...
";
    ...
    // ─── HintedCoverageReport::hint_iter_by_surface /
    // ─── hint_count_by_surface — the surface-projected pair of the
    // ─── sub-report enumeration primitive, jointly welded to
    // ─── HealthReport's own-surface projections on the two coverage
    // ─── surfaces (DeadKnob, StaleEntry) and pinned to zero on the two
    // ─── surfaces this sub-report carries no hints on (ValueKey,
    // ─── EnvVar). ───

    #[test]
    fn hinted_coverage_hint_iter_by_surface_count_equals_hint_count_by_surface() {
        // The delegation-shape invariant on the sub-report
        // surface-projected pair: `hint_iter_by_surface(s).count()`
        // equals `hint_count_by_surface(s)` on any input, for every
        // `s ∈ HintSurface::ALL`. Welds the sub-report iter-side peer
        // to its count-side peer through the shared `.filter(|h|
        // h.surface == s)` on the enumeration primitive `hint_iter`,
        // so a future refactor that inlines one against the other
        // must keep the two in lockstep. Verified on BOTH the clean
        // corner (every surface count 0) and a mixed corner where
        // BOTH own-surface arms of the bidirectional diff are nonzero
        // (so neither DeadKnob nor StaleEntry is vacuously covered),
        // and on every one of HintSurface::ALL's four variants (so
        // the two foreign-surface arms are not vacuously covered).
        let clean_consumed = &["name", "tags", "window.width", "window.height"];
        let clean_hinted = ConfigCoverage::hinted_report::<Demo>(clean_consumed);
        for &surface in HintSurface::ALL {
            assert_eq!(
                clean_hinted.hint_iter_by_surface(surface).count(),
                clean_hinted.hint_count_by_surface(surface),
        ...
        for &surface in HintSurface::ALL {
            assert_eq!(
                mixed_hinted.hint_iter_by_surface(surface).count(),
                mixed_hinted.hint_count_by_surface(surface),
    ...
    #[test]
    fn hinted_coverage_hint_count_by_surface_sums_to_hint_count_over_hint_surface_all() {
        // The fold-composition invariant on the sub-report count-side
        // peer: summing `hint_count_by_surface(s)` across
        // `HintSurface::ALL` recovers `hint_count()` with NO
        // cross-surface dedup — the same partition invariant the
        // whole-report `HealthReport::hint_count_by_surface_folds_to_hint_count`
        // test welds one scope up, projected here onto the sub-report's
        // two-variant sub-partition of HintSurface::ALL (with the two
        // foreign-to-this-sub-report surfaces contributing zero terms).
        // Verified on both the clean corner (sum = 0) and a mixed
        // corner where BOTH own-surface arms carry exactly one hint
        // (sum = 2), so no arm is vacuously covered and the partition
        // fact is real, not trivially green on a single-arm shape.
        let clean_consumed = &["name", "tags", "window.width", "window.height"];
        let clean_hinted = ConfigCoverage::hinted_report::<Demo>(clean_consumed);
        let clean_sum: usize = HintSurface::ALL
            .iter()
            .map(|s| clean_hinted.hint_count_by_surface(*s))
            .sum();
        assert_eq!(clean_sum, clean_hinted.hint_count());
        assert_eq!(clean_sum, 0);

        let mixed_consumed = &["name", "tags", "window.witdh", "window.height"];
        let mixed_hinted = ConfigCoverage::hinted_report::<Demo>(mixed_consumed);
        let mixed_sum: usize = HintSurface::ALL
            .iter()
            .map(|s| mixed_hinted.hint_count_by_surface(*s))
            .sum();
        assert_eq!(mixed_sum, mixed_hinted.hint_count());
        assert_eq!(mixed_sum, 2);
        assert_eq!(
            mixed_hinted.hint_count_by_surface(HintSurface::DeadKnob),
            mixed_hinted.dead_knobs.len(),
        );
        assert_eq!(
            mixed_hinted.hint_count_by_surface(HintSurface::StaleEntry),
            mixed_hinted.stale_entries.len(),
        );
    }

    #[test]
    fn hinted_coverage_hint_count_by_surface_is_zero_on_foreign_surfaces() {
        // The sub-report-bounded fact welded to the type: at the
        // HintedCoverageReport scope, `hint_count_by_surface(s)` is
        // ALWAYS 0 for `s ∈ {HintSurface::ValueKey, HintSurface::EnvVar}`
        // — the two variants of HintSurface::ALL this sub-report
        // carries no hints on (since `hint_iter` only ever tags with
        // DeadKnob or StaleEntry per the canonical yield order welded
        // by `hinted_coverage_hint_iter_yields_dead_knobs_first_then_stale_entries`).
        // The whole-report count-side peer accepts all four variants
        // uniformly (returning per-surface counts across the entire
        // HealthReport, including nonzero for ValueKey / EnvVar); the
        // sub-report count-side peer accepts the same four variants for
        // API symmetry but is BOUNDED to two by the sub-report's
        // structure. Verified on the dirtiest sub-report corner
        // available (both own-surface arms nonzero, both
        // `did_you_mean`-populated), so the zero on the two foreign
        // surfaces is a fact of the sub-report's shape and not a
        // vacuous consequence of a clean input.
        let consumed = &["name", "tags", "window.witdh", "window.height"];
        let hinted = ConfigCoverage::hinted_report::<Demo>(consumed);
        assert_eq!(hinted.hint_count(), 2);
        assert!(hinted.hint_count_by_surface(HintSurface::DeadKnob) > 0);
        assert!(hinted.hint_count_by_surface(HintSurface::StaleEntry) > 0);
        assert_eq!(hinted.hint_count_by_surface(HintSurface::ValueKey), 0);
        assert_eq!(hinted.hint_count_by_surface(HintSurface::EnvVar), 0);
        assert_eq!(
            hinted.hint_iter_by_surface(HintSurface::ValueKey).count(),
            0
        );
        assert_eq!(hinted.hint_iter_by_surface(HintSurface::EnvVar).count(), 0);
    }

    #[test]
    fn hinted_coverage_hint_iter_by_surface_matches_health_report_on_own_surfaces() {
        // The whole-report/sub-report compositional wiring at the
        // surface-projected layer: for `s ∈ {HintSurface::DeadKnob,
        // HintSurface::StaleEntry}` — the two own surfaces of this
        // sub-report — `hinted.hint_iter_by_surface(s)` yields the same
        // sequence as `health.hint_iter_by_surface(s)` on shared inputs.
        // The direct head-projection peer of
        // `hinted_coverage_hint_iter_matches_health_report_coverage_prefix`
        // one scope over on the surface-projected pair: the coverage
        // sub-report enumeration prefix identity, folded through the
        // per-surface `.filter` beneath both scopes. Welds
        // `HintedCoverageReport::hint_iter_by_surface` to its whole-
        // report peer, so a future refactor of either side that reroutes
        // a coverage-surface tag away from the coverage sub-report
        // breaks this weld on whichever corner exposes it. Verified
        // on a construction where BOTH the whole-report and sub-report
        // corners have real hints on both own surfaces (window.width
        // dead knob, window.witdh stale entry) AND the whole-report
        // carries additional hints on ValueKey / EnvVar (so the identity
        // is a real prefix, not a vacuous whole-equals-whole).
        let consumed = &["name", "tags", "window.witdh", "window.height"];
        let yaml = "\
name: kanchi
window:
...
";
        ...
        for &surface in &[HintSurface::DeadKnob, HintSurface::StaleEntry] {
            let sub: Vec<SurfaceHint<'_>> = health.coverage.hint_iter_by_surface(surface).collect();
            let whole: Vec<SurfaceHint<'_>> = health.hint_iter_by_surface(surface).collect();
            assert_eq!(sub, whole, "own surface={surface:?}");
            assert!(!sub.is_empty(), "own surface={surface:?}");
            assert_eq!(
                health.coverage.hint_count_by_surface(surface),
                health.hint_count_by_surface(surface),
                "own surface={surface:?}",
            );
        }
        // The two foreign surfaces still yield empty at the sub-report
        // scope even when the whole-report populates them — the sub-
        // report-bounded fact holds on a whole-report with hints on
        // every surface, not just on a coverage-only construction.
        for &surface in &[HintSurface::ValueKey, HintSurface::EnvVar] {
            assert_eq!(
                health.coverage.hint_iter_by_surface(surface).count(),
                0,
...
";
    ...
    // ─── hinted_coverage hint_counts_by_surface — plural batch peer
    // ─── of hint_count_by_surface at the sub-report scope ───────────

    #[test]
    fn hinted_coverage_hint_counts_by_surface_pairs_agree_with_hint_count_by_surface_pointwise() {
        // The pointwise delegation invariant on the sub-report count-
        // side plural batch peer: for every (surface, count) pair
        // returned, the tag equals HintSurface::ALL[i] verbatim
        // (order welded to the canonical listing) and the count
        // equals `hint_count_by_surface(surface)` verbatim
        // (delegation welded to the scalar sub-report peer, so a
        // future refactor that inlines one against the other must
        // keep the two in lockstep). Direct sub-report peer of
        // `hint_counts_by_surface_pairs_agree_with_hint_count_by_surface_pointwise`
        // one scope up. Verified on the clean corner (every count 0)
        // and a mixed corner where BOTH own-surface arms of the
        // bidirectional diff carry one hint each (own-surface counts
        // 1, foreign-surface counts 0), so no per-surface arm is
        // vacuously covered.
        let clean_consumed = &["name", "tags", "window.width", "window.height"];
        let clean_hinted = ConfigCoverage::hinted_report::<Demo>(clean_consumed);
        let clean_hist = clean_hinted.hint_counts_by_surface();
        assert_eq!(
            clean_hist.len(),
            HintSurface::ALL.len(),
        ...
        for (i, (surface, count)) in clean_hist.iter().enumerate() {
            ...
            assert_eq!(*count, clean_hinted.hint_count_by_surface(*surface));
            assert_eq!(*count, 0, "clean corner: every per-surface count is 0");
        }

        let mixed_consumed = &["name", "tags", "window.witdh", "window.height"];
        let mixed_hinted = ConfigCoverage::hinted_report::<Demo>(mixed_consumed);
        let mixed_hist = mixed_hinted.hint_counts_by_surface();
        assert_eq!(mixed_hist.len(), HintSurface::ALL.len());
        for (i, (surface, count)) in mixed_hist.iter().enumerate() {
            assert_eq!(*surface, HintSurface::ALL[i], "position {i}");
            assert_eq!(*count, mixed_hinted.hint_count_by_surface(*surface));
        }
        assert_eq!(mixed_hist[0], (HintSurface::DeadKnob, 1));
        assert_eq!(mixed_hist[1], (HintSurface::StaleEntry, 1));
        assert_eq!(mixed_hist[2], (HintSurface::ValueKey, 0));
        assert_eq!(mixed_hist[3], (HintSurface::EnvVar, 0));
    }

    #[test]
    fn hinted_coverage_hint_counts_by_surface_sum_equals_hint_count() {
        // The fold-composition invariant on the sub-report count-side
        // batch peer, promoted from `hint_count_by_surface`'s
        // pointwise sum-back-to-`hint_count`
        // (`hinted_coverage_hint_count_by_surface_sums_to_hint_count_over_hint_surface_all`)
        // to a whole-histogram fact: the sum of the array's counts
        // equals `hint_count()` with NO cross-surface dedup, on both
        // a clean corner (sum 0) and a mixed corner where BOTH
        // own-surface arms carry one hint (sum 2). A future refactor
        // that drops or duplicates one surface's entry in the
        // histogram turns this red on whichever corner exposes it.
        let clean_consumed = &["name", "tags", "window.width", "window.height"];
        let clean_hinted = ConfigCoverage::hinted_report::<Demo>(clean_consumed);
        let clean_sum: usize = clean_hinted
        ...
        let mixed_consumed = &["name", "tags", "window.witdh", "window.height"];
        let mixed_hinted = ConfigCoverage::hinted_report::<Demo>(mixed_consumed);
        let mixed_sum: usize = mixed_hinted
            .hint_counts_by_surface()
            .iter()
            .map(|(_, n)| *n)
            .sum();
    ...
    #[test]
    fn hinted_coverage_hint_counts_by_surface_tag_sequence_equals_hint_surface_all() {
        // The tag-order invariant on the sub-report batch peer,
        // verified independently of the counts: the sequence of
        // surface tags in the returned array equals `HintSurface::ALL`
        // verbatim on any corner (a clean sub-report reveals ordering
        // regressions even when every count is 0). Direct sub-report
        // peer of `hint_counts_by_surface_tag_sequence_equals_hint_surface_all`
        // one scope up — a future reorder on any of the three (batch
        // peer, ALL const, hint_iter) turns red at the earliest seam.
        let hinted = ConfigCoverage::hinted_report::<Demo>(&[
            "name",
            "tags",
            "window.width",
        ...
        let observed: Vec<HintSurface> = hinted
            .hint_counts_by_surface()
            .iter()
            .map(|(s, _)| *s)
            .collect();
    ...
    #[test]
    fn hinted_coverage_hint_counts_by_surface_length_matches_hint_surface_all_length() {
        ...
        // silently return fewer than every surface) turns red — the
        // sub-report peer of
        ...
        assert_eq!(
            hinted.hint_counts_by_surface().len(),
            HintSurface::ALL.len()
        );
    }

    #[test]
    fn hinted_coverage_hint_counts_by_surface_is_zero_on_foreign_surfaces() {
        // The sub-report-bounded fact promoted from the scalar peer's
        // `hinted_coverage_hint_count_by_surface_is_zero_on_foreign_surfaces`
        // to the plural batch peer: at the HintedCoverageReport
        // scope, the two [`HintSurface::ValueKey`] /
        // [`HintSurface::EnvVar`] entries of the returned array are
        // ALWAYS `(surface, 0)`, on the dirtiest sub-report corner
        // available (both own-surface arms nonzero, both
        // `did_you_mean`-populated). The whole-report batch peer's
        // per-surface counts depend on the whole-report count on
        // those surfaces (nonzero ValueKey / EnvVar counts show up in
        // its histogram); the sub-report batch peer is BOUNDED to
        // zero on those two entries by the sub-report's structure. So
        // the sub-report histogram carries a `(0, 0)` foreign pair on
        // ANY sub-report — a golden fixture that pins the two
        // foreign entries as `0` cannot be defeated by a whole-report
        // input.
        let consumed = &["name", "tags", "window.witdh", "window.height"];
        let hinted = ConfigCoverage::hinted_report::<Demo>(consumed);
        assert_eq!(hinted.hint_count(), 2);
        let hist = hinted.hint_counts_by_surface();
        // Own-surface entries are nonzero (the sub-report scope
        // carries hints on both own arms), so the two foreign zeros
        // are a bounded fact and not vacuous.
        assert!(hist[0].1 > 0, "DeadKnob entry must be nonzero: {hist:?}");
        assert!(hist[1].1 > 0, "StaleEntry entry must be nonzero: {hist:?}");
        assert_eq!(hist[2], (HintSurface::ValueKey, 0));
        assert_eq!(hist[3], (HintSurface::EnvVar, 0));
    }

    #[test]
    fn hinted_coverage_hint_counts_by_surface_matches_health_report_on_own_surfaces() {
        // The whole-report/sub-report compositional wiring at the
        // count-side plural batch layer: for `s ∈ {HintSurface::DeadKnob,
        // HintSurface::StaleEntry}` — the two own surfaces of this
        // sub-report — the corresponding entry of
        // `hinted.hint_counts_by_surface()` equals the corresponding
        // entry of `health.hint_counts_by_surface()` on shared
        // inputs. The direct count-side batch peer of
        // `hinted_coverage_is_clean_by_surface_matches_health_report_on_own_surfaces`
        // one axis over on the surface-projected bool side, and the
        // plural batch peer of the identity
        // `hinted_coverage_hint_iter_by_surface_matches_health_report_on_own_surfaces`
        // welds on the enumeration side. A future refactor of either
        // side that reroutes a coverage-surface tag away from the
        // coverage sub-report breaks this weld on whichever corner
        // exposes it. Verified on a construction where BOTH the
        // whole-report and sub-report corners have real hints on both
        // own surfaces AND the whole-report carries additional hints
        // on ValueKey / EnvVar (so the identity is a real own-surface
        // fact, not a vacuous whole-equals-whole with both scopes
        // trivially clean).
        let consumed = &["name", "tags", "window.witdh", "window.height"];
        let yaml = "\
name: kanchi
...
";
        ...
        let env: Vec<(String, String)> = vec![("MYAPP_ENV_ONLY_LEAF".into(), "x".into())];
        let health = ConfigCoverage::health_report::<Demo, _, _>(consumed, &value, "MYAPP_", &env);
        let sub_hist = health.coverage.hint_counts_by_surface();
        let whole_hist = health.hint_counts_by_surface();
        // Same tag order (both fold through HintSurface::ALL), so
        // pointwise indexing lines up own-surface (i=0,1) with
        // own-surface and foreign-surface (i=2,3) with foreign-
        // surface across the two scopes.
        ...
        // count equals whole-report count and both are nonzero.
        for i in 0..2 {
            assert_eq!(sub_hist[i].1, whole_hist[i].1, "own position {i}");
            assert!(
                sub_hist[i].1 > 0,
                "own position {i}: sub-report count must be nonzero for this test's premise",
            );
        }
        // Foreign-surface positions (ValueKey, EnvVar): sub-report
        // count is 0 (bounded fact) while whole-report count is
        // nonzero — the sub-report's foreign zero holds on a whole-
        // report carrying hints on every surface, not just on a
        // coverage-only construction.
        for i in 2..HintSurface::ALL.len() {
            assert_eq!(sub_hist[i].1, 0, "foreign position {i}: sub-report");
            assert!(
                whole_hist[i].1 > 0,
                "foreign position {i}: whole-report count must be nonzero for this test's premise",
            );
        }
    }

    #[test]
    fn hinted_coverage_is_clean_by_surfaces_pairs_agree_with_is_clean_by_surface_pointwise() {
        // The pointwise delegation invariant on the sub-report bool-side
        // plural batch peer: for every (surface, clean) pair returned,
        // the tag equals HintSurface::ALL[i] verbatim (order welded to
        // the canonical listing) and the predicate equals
        // `is_clean_by_surface(surface)` verbatim (delegation welded to
        // the scalar sub-report peer, so a future refactor that inlines
        // one against the other must keep the two in lockstep). Direct
        // sub-report peer of
        // `is_clean_by_surfaces_pairs_agree_with_is_clean_by_surface_pointwise`
        // one scope up. Verified on the clean corner (every predicate
        // true) and a mixed corner where BOTH own-surface arms of the
        // bidirectional diff carry one hint each (own-surface predicates
        // false, foreign-surface predicates true), so no per-surface arm
        // is vacuously covered.
        let clean_consumed = &["name", "tags", "window.width", "window.height"];
        let clean_hinted = ConfigCoverage::hinted_report::<Demo>(clean_consumed);
        let clean_hist = clean_hinted.is_clean_by_surfaces();
        assert_eq!(
            clean_hist.len(),
            HintSurface::ALL.len(),
            "is_clean_by_surfaces must return one entry per HintSurface::ALL variant",
        );
        for (i, (surface, clean)) in clean_hist.iter().enumerate() {
            assert_eq!(*surface, HintSurface::ALL[i], "position {i}");
            assert_eq!(*clean, clean_hinted.is_clean_by_surface(*surface));
            assert!(*clean, "clean corner: every per-surface predicate is true");
        }

        let mixed_consumed = &["name", "tags", "window.witdh", "window.height"];
        let mixed_hinted = ConfigCoverage::hinted_report::<Demo>(mixed_consumed);
        let mixed_hist = mixed_hinted.is_clean_by_surfaces();
        assert_eq!(mixed_hist.len(), HintSurface::ALL.len());
        for (i, (surface, clean)) in mixed_hist.iter().enumerate() {
            assert_eq!(*surface, HintSurface::ALL[i], "position {i}");
            assert_eq!(*clean, mixed_hinted.is_clean_by_surface(*surface));
        }
        assert_eq!(mixed_hist[0], (HintSurface::DeadKnob, false));
        assert_eq!(mixed_hist[1], (HintSurface::StaleEntry, false));
        assert_eq!(mixed_hist[2], (HintSurface::ValueKey, true));
        assert_eq!(mixed_hist[3], (HintSurface::EnvVar, true));
    }

    #[test]
    fn hinted_coverage_is_clean_by_surfaces_and_equals_is_clean() {
        // The fold-composition invariant on the sub-report bool-side
        // batch peer, promoted from `is_clean_by_surface`'s pointwise
        // AND-back-to-`is_clean`
        // (`hinted_coverage_is_clean_iff_every_is_clean_by_surface`) to
        // a whole-histogram fact: the AND of the array's predicates
        // equals `is_clean()`. Verified across the sub-report's joint
        // predicate lattice — clean corner (whole clean / every entry
        // true), both-own-surfaces-dirty corner (whole dirty / both
        // own-surface entries false, both foreign entries still true),
        // dead-knob-only corner (whole dirty / only DeadKnob false),
        // stale-entry-only corner (whole dirty / only StaleEntry false)
        // — so no arm is vacuously covered and the shape of the array
        // (not just its total) matters most.
        let clean_consumed = &["name", "tags", "window.width", "window.height"];
        let clean_hinted = ConfigCoverage::hinted_report::<Demo>(clean_consumed);
        let clean_and: bool = clean_hinted.is_clean_by_surfaces().iter().all(|(_, c)| *c);
        assert_eq!(clean_and, clean_hinted.is_clean());
        assert!(clean_and, "clean corner: whole-histogram AND must be true");

        let mixed_consumed = &["name", "tags", "window.witdh", "window.height"];
        let mixed_hinted = ConfigCoverage::hinted_report::<Demo>(mixed_consumed);
        let mixed_hist = mixed_hinted.is_clean_by_surfaces();
        let mixed_and: bool = mixed_hist.iter().all(|(_, c)| *c);
        assert_eq!(mixed_and, mixed_hinted.is_clean());
        assert!(
            !mixed_and,
            "mixed corner: whole-histogram AND must be false when any surface is dirty",
        );
        // Sub-report-bounded shape: both own surfaces false, both
        // foreign surfaces true.
        assert_eq!(mixed_hist[0], (HintSurface::DeadKnob, false));
        assert_eq!(mixed_hist[1], (HintSurface::StaleEntry, false));
        assert_eq!(mixed_hist[2], (HintSurface::ValueKey, true));
        assert_eq!(mixed_hist[3], (HintSurface::EnvVar, true));

        // Dead-knob-only corner: only DeadKnob false, every other
        // surface stays true (StaleEntry clean at the sub-report scope,
        // ValueKey / EnvVar clean by the sub-report-bounded fact).
        let dead_only_consumed = &["window.width"];
        let dead_only_hinted = ConfigCoverage::hinted_report::<Demo>(dead_only_consumed);
        let dead_only_hist = dead_only_hinted.is_clean_by_surfaces();
        let dead_only_and: bool = dead_only_hist.iter().all(|(_, c)| *c);
        assert_eq!(dead_only_and, dead_only_hinted.is_clean());
        assert!(
            !dead_only_and,
            "dead-only corner: whole-histogram AND must be false"
        );
        assert_eq!(dead_only_hist[0], (HintSurface::DeadKnob, false));
        assert_eq!(dead_only_hist[1], (HintSurface::StaleEntry, true));
        assert_eq!(dead_only_hist[2], (HintSurface::ValueKey, true));
        assert_eq!(dead_only_hist[3], (HintSurface::EnvVar, true));

        // Stale-entry-only corner: only StaleEntry false, every other
        // surface stays true.
        let stale_only_consumed = &["name", "tags", "window.width", "window.height", "bogus"];
        let stale_only_hinted = ConfigCoverage::hinted_report::<Demo>(stale_only_consumed);
        let stale_only_hist = stale_only_hinted.is_clean_by_surfaces();
        let stale_only_and: bool = stale_only_hist.iter().all(|(_, c)| *c);
        assert_eq!(stale_only_and, stale_only_hinted.is_clean());
        assert!(
            !stale_only_and,
            "stale-only corner: whole-histogram AND must be false",
        );
        assert_eq!(stale_only_hist[0], (HintSurface::DeadKnob, true));
        assert_eq!(stale_only_hist[1], (HintSurface::StaleEntry, false));
        assert_eq!(stale_only_hist[2], (HintSurface::ValueKey, true));
        assert_eq!(stale_only_hist[3], (HintSurface::EnvVar, true));
    }

    #[test]
    fn hinted_coverage_is_clean_by_surfaces_tag_sequence_equals_hint_surface_all() {
        // The tag-order invariant on the sub-report bool-side batch
        // peer, verified independently of the predicates: the sequence
        // of surface tags in the returned array equals `HintSurface::ALL`
        // verbatim on any corner (a clean sub-report reveals ordering
        // regressions even when every predicate is true). Direct
        // sub-report peer of
        // `is_clean_by_surfaces_tag_sequence_equals_hint_surface_all`
        // one scope up — a future reorder on any of the three (batch
        // peer, ALL const, is_clean_by_surface) turns red at the
        // earliest seam.
        let hinted = ConfigCoverage::hinted_report::<Demo>(&[
            "name",
            "tags",
    ...
    #[test]
    fn hinted_coverage_is_clean_by_surfaces_length_matches_hint_surface_all_length() {
        // The array's type-level cardinality — const-evaluated from
        // `HintSurface::ALL.len()` at the return-type declaration —
        // must equal the const's own length at runtime, so a consumer
        // that destructures the array via a
        ...
        // silently return fewer than every surface) turns red — the
        // sub-report peer of
        // `is_clean_by_surfaces_length_matches_hint_surface_all_length`
        // one scope up.
        let hinted = ConfigCoverage::hinted_report::<Demo>(&[
            "name",
            "tags",
        ...
        assert_eq!(hinted.is_clean_by_surfaces().len(), HintSurface::ALL.len());
    }

    #[test]
    fn hinted_coverage_is_clean_by_surfaces_is_true_on_foreign_surfaces() {
        // The sub-report-bounded fact promoted from the scalar peer's
        // `hinted_coverage_is_clean_by_surface_is_true_on_foreign_surfaces`
        // to the plural batch peer: at the HintedCoverageReport scope,
        // the two [`HintSurface::ValueKey`] / [`HintSurface::EnvVar`]
        // entries of the returned array are ALWAYS `(surface, true)`,
        // on the dirtiest sub-report corner available (both own-surface
        // arms nonzero, both `did_you_mean`-populated). The whole-report
        // batch peer's per-surface predicates depend on the whole-report
        // count on those surfaces (nonzero ValueKey / EnvVar counts flip
        // them false in its histogram); the sub-report batch peer is
        // BOUNDED to true on those two entries by the sub-report's
        // structure. So the sub-report histogram carries a
        // `(true, true)` foreign pair on ANY sub-report — a golden
        // fixture that pins the two foreign entries as `true` cannot be
        // defeated by a whole-report input.
        let consumed = &["name", "tags", "window.witdh", "window.height"];
        let hinted = ConfigCoverage::hinted_report::<Demo>(consumed);
        assert_eq!(hinted.hint_count(), 2);
        let hist = hinted.is_clean_by_surfaces();
        // Own-surface entries are false (the sub-report scope carries
        // hints on both own arms), so the two foreign trues are a
        // bounded fact and not vacuous.
        assert!(!hist[0].1, "DeadKnob entry must be false: {hist:?}");
        assert!(!hist[1].1, "StaleEntry entry must be false: {hist:?}");
        assert_eq!(hist[2], (HintSurface::ValueKey, true));
        assert_eq!(hist[3], (HintSurface::EnvVar, true));
    }

    #[test]
    fn hinted_coverage_is_clean_by_surfaces_matches_health_report_on_own_surfaces() {
        // The whole-report/sub-report compositional wiring at the
        // bool-side plural batch layer: for `s ∈ {HintSurface::DeadKnob,
        // HintSurface::StaleEntry}` — the two own surfaces of this
        // sub-report — the corresponding entry of
        // `hinted.is_clean_by_surfaces()` equals the corresponding entry
        // of `health.is_clean_by_surfaces()` on shared inputs. The
        // direct bool-side batch peer of
        // `hinted_coverage_is_clean_by_surface_matches_health_report_on_own_surfaces`
        // one axis over on the surface-projected scalar peer, and the
        // plural batch peer of the identity
        // `hinted_coverage_hint_counts_by_surface_matches_health_report_on_own_surfaces`
        // welds on the count side. A future refactor of either side
        // that reroutes a coverage-surface tag away from the coverage
        // sub-report breaks this weld on whichever corner exposes it.
        // Verified on a construction where BOTH the whole-report and
        // sub-report corners have real hints on both own surfaces AND
        // the whole-report carries additional hints on ValueKey /
        // EnvVar (so the identity is a real own-surface fact, not a
        // vacuous whole-equals-whole with both scopes trivially clean,
        // and the two foreign-surface predicates flip apart across
        // scopes — true at the sub-report scope, false at the whole-
        // report scope — which the test also pins as a live premise).
        let consumed = &["name", "tags", "window.witdh", "window.height"];
        let yaml = "\
name: kanchi
window:
...
";
        ...
        let env: Vec<(String, String)> = vec![("MYAPP_ENV_ONLY_LEAF".into(), "x".into())];
        let health = ConfigCoverage::health_report::<Demo, _, _>(consumed, &value, "MYAPP_", &env);
        let sub_hist = health.coverage.is_clean_by_surfaces();
        let whole_hist = health.is_clean_by_surfaces();
        // Same tag order (both fold through HintSurface::ALL), so
        // pointwise indexing lines up own-surface (i=0,1) with
        ...
        // Own-surface positions (DeadKnob, StaleEntry): sub-report
        // predicate equals whole-report predicate and both are false.
        for i in 0..2 {
            assert_eq!(sub_hist[i].1, whole_hist[i].1, "own position {i}");
            assert!(
                !sub_hist[i].1,
                "own position {i}: sub-report predicate must be false for this test's premise",
            );
        }
        // Foreign-surface positions (ValueKey, EnvVar): sub-report
        // predicate is true (bounded fact) while whole-report predicate
        // is false — the sub-report's foreign true holds on a whole-
        // report carrying hints on every surface, not just on a
        // coverage-only construction.
        for i in 2..HintSurface::ALL.len() {
            assert!(sub_hist[i].1, "foreign position {i}: sub-report");
            assert!(
                !whole_hist[i].1,
                "foreign position {i}: whole-report predicate must be false for this test's premise",
            );
        }
    }

    #[test]
    fn hinted_coverage_is_clean_by_surfaces_pairs_agree_with_hint_counts_by_surface_pointwise() {
        // The bool-side vs count-side agreement of the two sub-report
        // histogram peers: for every surface, the bool entry equals the
        // count entry being zero (both projections of `hint_iter`
        // partitioned by surface at the coverage sub-report scope).
        // Pins the two sub-report histograms — which could otherwise
        // silently drift if one is refactored to a shortcut that skips
        // a surface — to the same underlying per-surface fold. Direct
        // sub-report peer of
        // `is_clean_by_surfaces_pairs_agree_with_hint_counts_by_surface_pointwise`
        // one scope up. Verified on a mixed corner where both
        // own-surface arms are dirty, so at least one entry lands on
        // the dirty side of the zero-count / clean-predicate boundary
        // (the two foreign entries always land on the clean side by
        // the sub-report-bounded fact).
        let mixed_consumed = &["name", "tags", "window.witdh", "window.height"];
        let mixed_hinted = ConfigCoverage::hinted_report::<Demo>(mixed_consumed);
